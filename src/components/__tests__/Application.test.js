...
describe("Application", () => {
  ...
  it("3 loads data, cancels an interview and increases the spots remaining for Monday by 2", async () => {
    const { container } = render(<Application />);
  ...
  it("5 shows the save error when failing to save an appointment", async () => {
    ...
    await waitForElement(() => getByText(bookedAppointment, /error save/i));
  });
  ...
  it("6 shows the delete error when failing to delete an existing appointment", async () => {
    ...
    await waitForElement(() => getByText(bookedAppointment, /error delete/i));
  });
});
